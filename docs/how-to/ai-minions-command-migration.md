...
| *(new)* — | `ai-minions tui` | Interactive cockpit MVP (TTY) with run selector, status pane, evidence/attach pane, and config/credentials readiness pane; `--run-id`/`--latest`/`--file` keep read-only evidence panels |
