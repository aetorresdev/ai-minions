...
| *(new)* — | `ai-minions tui` | Interactive cockpit MVP (TTY) with run selector, status pane, and evidence/attach pane; `--run-id`/`--latest`/`--file` keep read-only evidence panels |
