...
5. **UX discovery (parallel):** while cohort runs, collect friction evidence per [cohort-ux-discovery-runbook](cohort-ux-discovery-runbook.md). Operator UX cut ships in **`v0.24.0-beta.1`**; tester evidence + Operator TUI MVP ship in **`v0.25.0-beta.1`**. Friction logs remain for ongoing cohort quality — not a substitute for `LIVE_PASS`.
