...
**v0.15 gate criterion:** document and track **minimum smoke evidence** across OS × provider × flow before any **external usability beta** (**v0.20.0-beta.1**). Manual attestation is acceptable in v0.15 — full CI grid automation is **out of scope**.
...
Required before **external beta** (**v0.20.0-beta.1**). Experimental cells may stay `EXCEPTION` until a backend ships.
...
| `linux-openai-compat-sa-trivial` | linux | openai-compat-local | single-agent | trivial | experimental | PENDING | | EXCEPTION allowed pre-v0.20.0-beta.1 |
