...
| 5b | **Skills (Claude Code)** | If agents will invoke `Skill`, which skill ids are in scope for the active MODE role. | `orchestrator/security/skill-registry.v1.json`; opt-in hook `ORCH_SKILL_REGISTRY_ENFORCE=1` — [skill-registry-contract.md](skill-registry-contract.md). |
