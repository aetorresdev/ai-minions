...
- **Splash / brand screen (first paint):** Cerberus brand splash — Semantic Guardians arcade art by default (Neon via `AI_MINIONS_TUI_GUARDIAN=neon`; textual Cerberus when art resolves to text), `AI-MINIONS` wordmark, Validate / Trace / Enforce markers, version, continue-to-shell (`AI_MINIONS_TUI_SKIP_SPLASH=1` skips; auto-dismisses after a short timer or any key — Esc / `q` also continues). **Cold start drains leftover stdin twice:** before the first Ink mount (so residual Enter/`1` cannot auto-dismiss the brand splash) and again after readiness/run discovery, immediately before the first interactive shell mount (so keys buffered during discovery cannot skip landing into Start New Run); a safety-ceiling truncation at either drain aborts the boot with `TUI_SHELL_COLD_START_DRAIN_TRUNCATED` instead of mounting with leftovers. First paint uses a bounded minimal model (version + explicit `loading`/`unavailable` readiness only) — credential/path assessment and run/trace discovery run only after splash continuation, then the shell remounts with populated state. Short / tight TTYs keep the frame within the reported viewport (demote spacers / pixel wordmark before clipping brand art) so first paint does not pad to 24 rows. Presentation only — not a capability claim; splash polish alone does not close the release Phase B gate.
...
Nested readline panes (guided launcher rollback, run selector, interactive doctor refresh, attach generation) may still use their own in-pane keys after a soft handoff; those are pane UX, not the top-level shell matrix. Fullscreen Overview / Explain / Evidence / Monitor / Settings are Ink-local (not nested).

**Ink-local surfaces (no unmount):** `home`, `help`, `diagnostics`, **`status`** (Overview `o` / Explain `x`), **`monitor`** (`m` / lifecycle alias), **`evidence`** (`e`), and **`config`** (Settings `4`) switch `contentSurface` inside the live Ink mount. **Phase-1 native workflows** (`launcher` / `runs` / overview) also stay inside Ink. Hotkeys and slash aliases for these (`h`/`?`/`3`/`4`/`o`/`x`/`e`/`m`/`2`/`1`, `/help`, `/runs`, `/new`, `/home`) must **not** soft-handoff / clear / remount — that looks like a silent quit (`TUI_SHELL_OK`). Nested panes remain only for actions that still need readline (attach generation, legacy select, interactive doctor refresh via slash/`ai-minions doctor`). `/help` shows slash vocabulary as an in-mount `action_result` (no remount). Landing **Quick Start** lists task goals `1`–`5` only (Home stays on Navigate / hotkey `h`) so ↑/↓ counts match the labeled digits. **Cold start** always boots to landing `home` (drain leftover stdin **before** the first Ink mount — brand splash or shell — **and again after discovery, immediately before the interactive shell mount**; ignore stale launcher resume) unless the operator explicitly opens Start New Run.
...
| `monitor` (`m`, contextual) | Live monitor — **Ink-local** `contentSurface=monitor` via `adaptLiveMonitor` / seeded lifecycle snapshot (**in-process**, no remount). Fresh live probe: CLI `ai-minions status` or slash `/status` (slash may soft-handoff; seeded `m` hotkey must not) |
...
**Fullscreen:** contextual **`m` / Monitor** when a run is selected (aliases in dispatch: `live`, `live-monitor`, `run-monitor`). **Ink-local** — switches `contentSurface` to `monitor` inside the live mount (same no-unmount contract as Overview / Evidence / Explain). Must not soft-handoff / clear / remount on the hotkey path.

- Uses the selected run when present; otherwise may prompt for run-id on nested/legacy paths only (Enter accepts previously selected run).
- Seeded fullscreen path reuses shell lifecycle / status snapshot fields; fresh CLI reads the same authoritative status/trace snapshot as `ai-minions status --json`.
