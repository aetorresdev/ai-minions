...
- **Splash / brand screen (first paint):** Cerberus brand splash — Semantic Guardians arcade art by default (Neon via `AI_MINIONS_TUI_GUARDIAN=neon`; textual Cerberus when art resolves to text), `AI-MINIONS` wordmark, Validate / Trace / Enforce markers, version, continue-to-shell (`AI_MINIONS_TUI_SKIP_SPLASH=1` skips; auto-dismisses after a short timer or any key — Esc / `q` also continues). **Cold start drains leftover stdin before the first Ink mount** so residual Enter/`1` cannot auto-dismiss the brand splash or skip landing into Start New Run. First paint uses a bounded minimal model (version + explicit `loading`/`unavailable` readiness only) — credential/path assessment and run/trace discovery run only after splash continuation, then the shell remounts with populated state. Short / tight TTYs keep the frame within the reported viewport (demote spacers / pixel wordmark before clipping brand art) so first paint does not pad to 24 rows. Presentation only — not a capability claim; splash polish alone does not close the release Phase B gate.
