...
- **Resize:** shell chrome (non-home) columns &lt; 72 → narrow (stacked). **Landing** composition: ≥100 cols and ≥24 rows → wide (guardian left · primary right · readiness/runs below); 80–99 cols → mid (reduced/stacked guardian); &lt;80 cols or short TTY → compact one-column (drop decorative art before action/state). `NO_COLOR` keeps hierarchy via labels, borders, focus markers, and explicit state text.
