...
**Activation:** see [activation.md](activation.md). Host hooks and ai-minions role/gate machinery require `AI_MINIONS_ACTIVE=1` from the product CLI. `CLAUDE.md` and `MODE`/`FLOW` text are not activators.
...
| Hooks (`scripts/hooks/`) | Only when `AI_MINIONS_ACTIVE=1` (else no-op) | Gate events logged; `advance_mode` blocked if `compact_handoff` not called; mode/QA/handoff violations surfaced |
