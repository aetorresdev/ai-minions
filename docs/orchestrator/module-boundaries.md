...
| **model-runtime** | `modules/model-runtime/` (policy/tier gate, runners, routing table, `local-model-*.js`, `runner-model-routing.js`, `flow-hook-bridge.js`) · root shims · legacy shims at `agents/runtime/*`, `agents/routing/model-routing.js` |
