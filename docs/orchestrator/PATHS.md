...
| MODE agents (Node) | **`orchestrator/agents.js`** — canonical `require("./agents")` facade. **`orchestrator/agents/`** — e.g. `routing/model-routing.js`, `permissions.js`, `validate-output.js`, `registry.js` (`buildAgents`), `runtime/*`, `prompts/ollama-appends.js`; not the same as repo-root **`agents/`** (subagent markdown for skills). |
...
- The string may still appear in **local planning notes** when describing the migration — not as a runnable path.
