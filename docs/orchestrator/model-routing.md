...
- **Bounded compact retry:** when the first reply has **no content, no `tool_calls`, and no `tools_used`**, `askAgent()` issues **one** compact retry (task + working directory only, no context block). The attempt is marked **before** the second call, so both success and failure paths carry `ollama_retried_after_empty: 1` in `context_stats` / trace.
