...
- **Today:** only **`"2"`** is published for this repository. It labels the JSON Schema at `orchestrator/schemas/trace-v2-line.schema.json` (Ajv validation at write time).
...
|--------|----------------|--------|
| Remove or rename a **required** field; change a field **type**; **tighten** validation (e.g. new required subfield) | **Breaking** | New **major** label (e.g. `"2"` → `"3"`), new JSON Schema file, bump **`TRACE_LINE_WRITER_VERSION`** in `orchestrator/trace-schema.js` (and bundled schema enum), update this doc and `strict-mode.md` in the **same** change. |
...
- **Constants:** `TRACE_LINE_WRITER_VERSION` and `SUPPORTED_TRACE_SCHEMA_VERSIONS_FOR_READ` in `orchestrator/trace-schema.js` — extend the `Set` when this binary ships multiple readers.
...
2. Update or add JSON Schema under `orchestrator/schemas/`.
...
5. Update **`orchestrator/README.md`** if the user-facing contract string changes.
...
Strict validation surfaces these as Ajv errors; see tests in `orchestrator/tests/traceSchema.test.js`.

## explain-run consumers

`orchestrator/explain-run.js` reads trace JSONL and derives a human or JSON summary. It is a **read-only consumer** — it does not validate against the JSON Schema at read time (best-effort parse: invalid lines are skipped). It reads these fields:
...
| `failure_type` | `iteration_done` (when present) | Coarse enum at write time for `outcome` ≠ `done`; pair with **`transition_reason.reason_code`** for meaningful breakdown. `explain-run` copies verbatim or uses `UNKNOWN` when a failed run omits the field |
...
| Artifact | Path |
|----------|------|
| JSON Schema (v2 line) | `orchestrator/schemas/trace-v2-line.schema.json` |
| Validator + policy | `orchestrator/trace-schema.js` (`TRACE_LINE_WRITER_VERSION`, `validateTraceLine`) |
| Write-time alias | `TRACE_SCHEMA_VERSION` in `orchestrator/orchestrator.js` (= `TRACE_LINE_WRITER_VERSION`) |
