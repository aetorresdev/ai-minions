...
- **Relationship to TUI:** The Ink shell uses deterministic **arcade / pixel-art** cell sprites (visual SoT: `assets/mockups/tui-landing-pixel-v1.png`). This SVG is the **official off-TTY / GUI** source (visual SoT: `assets/mockups/gui-landing-detailed-v1.png`). PNG 1×/2× exports may be generated only when an external platform requires raster assets; they are not the TUI runtime path. TUI sprites must **never** overwrite this master SVG.
