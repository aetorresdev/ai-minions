...
| **`ORCH_BUDGET_WARNING_RATIO`** | Optional ratio **`0 < value <= 1`**. When `ORCH_MAX_COST_USD` and USD rates are set, emits `budget_warning` once per phase after estimated actual spend reaches `ORCH_MAX_COST_USD * ratio` and before hard-stop. Trace-only; it does not block. Values outside range are ignored. |
| **`ORCH_BUDGET_LIMITS_JSON`** | Optional JSON object for actual-token budget limits: `{ "run": 2.5, "roles": { "qa": 1.0 }, "steps": { "<step_id>": 0.2 }, "models": { "ollama/qwen2.5-coder:7b": 1.5 } }`. `ORCH_MAX_COST_USD` is treated as `run` if JSON omits it. Hard-stop emits one canonical `GUARD_COST_LIMIT`; `triggered_budgets` lists every crossed scope; `budget_scope` uses precedence **step > role > model > run**. Invalid JSON emits `budget_config_invalid` and is ignored. |
...
# Harness resilience fixture evals (deterministic — no Ollama, no live MCP):
npm run test:eval:chaos-tool-failure   # tool_failure_eval scenarios (6 rows)
npm run test:eval:untrusted-context      # context_authority_check fixture corpus
npm run test:eval:context-authority      # runtime gate + REDTEAM integration tests
npm run test:eval:harness-resilience     # all three suites above
```
...
|----------|--------|---------|
| `orchestrator-unit-tests.yml` (`name: orchestrator-unit-tests`) | GitHub cloud | **PRs** to `main`/`master` when paths match (`orchestrator/**`, `scripts/hooks/**`, runtime policy, or this workflow). Parallel jobs: **`lint`** (Node **22** only — `lint:static` + `test:hooks`) and **`unit`** (Node **22** + **24** — `test:node` + TUI quality + routing-release fixture). `workflow_dispatch` supported. Harness-scope step runs `orchestrator/scripts/ci-check-harness-scope.sh`. **Check-name change:** former matrix job `lint-and-unit (N)` → `lint` + `unit (N)`. Branch protection currently has no required status-check contexts; if you add them later, require both `lint` and `unit (22)` (and optionally `unit (24)`). |
| `ship-fresh-checkout-smoke.yml` (`name: SHIP fresh checkout smoke`) | GitHub cloud | **`workflow_dispatch` only** — full local gate (`npm ci` + `npm test` in `orchestrator/`) on a clean checkout for **alpha / ship-ready evidence** (`docs/orchestrator/alpha-release-checklist.md`). Does **not** run strict E2E (Ollama + MCP venv); use `orchestrator-e2e.yml` or a documented operator run for that gate. |
| `orchestrator-e2e.yml` | Self-hosted (`ollama` label) | PR when orchestrator core, `mcp-direct.py`, `tests/**`, `package.json` / lockfile, MCP server dirs, or this workflow change; **`workflow_dispatch`** (inputs `ollama_model`, `run_first_shot_report`) |

The E2E workflow requires a self-hosted runner with labels **`self-hosted`** and **`ollama`**, Ollama at `localhost:11434`, and network for `astral-sh/setup-uv` + `npm ci`. **PR blocking gate:** **`npm run test:e2e`** then **`npm run test:e2e:strict`** (strict suite **without** `ORCH_TEST_SYSTEM_PATH_HARNESS`; optional harness: `npm run test:e2e:strict:harness` locally). **DEV first-shot metric** (`test:e2e:dev-first-shot-report`) runs only on **`workflow_dispatch`** (optional input) — it is informational and was ~9 min of serial Ollama wall-clock when left on every PR. Residual PR E2E time is mostly inherent Ollama latency under `--test-concurrency=1` (do not parallelize against a single local model without measuring contention). **Fork PRs:** the E2E job is skipped when the PR comes from a fork. GitHub’s rules for **required checks** allow successful / skipped / neutral in many setups when the workflow completed; validate once with a **real fork PR** and your branch protection. See `.github/workflows/orchestrator-e2e.yml` and `docs/orchestrator/strict-mode.md` § *GitHub Actions — orchestrator-e2e.yml*.
