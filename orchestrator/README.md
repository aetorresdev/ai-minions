...
| `ship-fresh-checkout-smoke.yml` (`name: SHIP fresh checkout smoke`) | GitHub cloud | **`workflow_dispatch` only** — same steps as lint + unit (`npm ci` + `npm test` in `orchestrator/`) on a clean checkout for **alpha / ship-ready evidence** (`docs/orchestrator/alpha-release-checklist.md`). Does **not** run strict E2E (Ollama + MCP venv); use `orchestrator-e2e.yml` or a documented operator run for that gate. |
