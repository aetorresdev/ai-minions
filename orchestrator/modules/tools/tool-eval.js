...
function runToolEvalScenario(scenario, opts = {}) {
  ...
  if (scenario.use_test_manifest) {
    ...
    const { loadProjectPolicy, mergeProjectPolicy } = require("../../security/load-project-policy");
    const { permissionDomainForClassification } = require("../../security/classified-invocation-permission-gate");
...
function validateToolManifestEntry(toolEntry, toolId) {
  const { validateManifestRoot } = require("../../security/load-tool-action-manifest");
  const wrapped = {
    version: "tool-action-manifest.orchestrator.v1",
