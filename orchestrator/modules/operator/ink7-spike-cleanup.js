...
/**
 * Terminal restoration helpers for the Ink 7 framework spike.
 * Tracks raw-mode / cursor / alternate-screen style mutations for deterministic tests.
 */
...
function createTerminalGuard(options = {}) {
  ...
  return {
    ...
    /**
     * Restore terminal after normal exit, Ctrl+C, renderer exception, or child failure.
     * Idempotent.
     */
    restore(reason = 'normal') {
      ...
      if (originalSetRawMode && rawMode) {
        try {
          ...
          mutations.push({ kind: 'setRawMode_error', value: String(err && err.message ? err.message : err) });
        }
      }
      ...
      try {
        ...
        mutations.push({ kind: 'restore_write_error', value: String(err && err.message ? err.message : err) });
      }
      if (originalSetRawMode) {
