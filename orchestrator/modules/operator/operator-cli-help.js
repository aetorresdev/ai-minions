...
function printAiMinionsCliHelp(options = {}) {
  ...
  const lines = [
    ...
    `  ${cmd("init")}       Validate host prereqs + model discovery + write .ai-minions config`,
    ...
    `  ${cmd("tui")}        Read-only evidence surface (stdout panels — no edits/approvals/reruns)`,
    ...
    `  ${cmd("resume")}     Honest resume capability probe (not implemented)`,
    "",
    ...
    "  - resume does not launch runs — RUN_RESUME_NOT_IMPLEMENTED with inspect alternatives",
    ...
    "  --latest                 Use newest trace in metrics/traces",
    "  --file <path>            Trace JSONL path (overrides --run / --run-id)",
    "  --json                   Structured panels (phase timeline, step graph, summaries)",
    "",
    h("Exit codes:"),
