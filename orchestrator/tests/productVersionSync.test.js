...
describe("product-version-sync", () => {
  it("normalizes changelog versions with v prefix", () => {
    assert.equal(normalizeProductVersion("0.26.0-beta.1"), "v0.26.0-beta.1");
    assert.equal(normalizeProductVersion("v0.26.0-beta.1"), "v0.26.0-beta.1");
  });

  it("reads latest tagged section after [Unreleased]", () => {
    const changelog = fs.readFileSync(CHANGELOG_PATH, "utf8");
    assert.equal(readLatestChangelogVersion(changelog), "v0.26.0-beta.1");
  });
