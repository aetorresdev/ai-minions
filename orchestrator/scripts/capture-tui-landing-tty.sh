...
SOURCE_TIP="$(git -C "$ROOT" rev-parse HEAD 2>/dev/null || echo unknown)"
INK="$(node -e "console.log(require('$ORCH/node_modules/ink/package.json').version)" 2>/dev/null || echo unknown)"
CHECKOUT_VERSION="$(node -e "console.log(require('$ORCH/package.json').version)" 2>/dev/null || echo unknown)"
...
bin_version() {
  local bin="$1"
  # Prefer --version; fall back to unknown (do not invent checkout SHA as runner version).
...
if [[ "$USE_INSTALLED" -eq 1 ]]; then
  ...
  RUNNER_VERSION="$CHECKOUT_VERSION"
...
const meta = {
  ...
  note: 'Real PTY typescript; not Ink renderToString fixtures. source_tip_sha is checkout HEAD; runner_* describes the process that produced the frame.',
};
