...
>
> **Tag snapshot reconciliation:** git tag `v0.22.0-beta.1` points to `52a7172`, whose **tree** still contains CHANGELOG header `[0.22.0-alpha.1]` from E22-6 release-prep. Semver correction (`[0.22.0-beta.1]`, `PRODUCT_VERSION`) is on **`master`** after the tag (`bbdc62b`+). **Operator truth:** GitHub release + `ai-minions about` + latest `master` CHANGELOG use **beta.1**; checking out the tag alone is not sufficient for version labeling — treat tag-tree CHANGELOG header as frozen historical errata.
